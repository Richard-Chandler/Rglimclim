...
c      DATA (ZBQLIX(I),I=1,43) /8.001441D7,5.5321801D8,
...
c       INIT = 1
c      ENDIF
c      TMPVAR1 = DMOD(DBLE(SEED),B)
c      ZBQLIX(1) = TMPVAR1
c      DO 100 I = 2,43
c       TMPVAR1 = ZBQLIX(I-1)*3.0269D4
c       TMPVAR1 = DMOD(TMPVAR1,B)       
c       ZBQLIX(I) = TMPVAR1
c 100  CONTINUE

*
*       Use a congruential generator to initialise the seed array
*
      M = 2.0D0**32
      ...
      TMPVAR1= M * TMPVAR1 / B
      Do I=2,43
       TMPVAR1 = DMOD(1664525.0D0*TMPVAR1 + 1013904223.0D0,M)
       ZBQLIX(I) = (TMPVAR1 / M) * B
      End Do
      CURPOS = 1
      ID22 = 22
      ID43 = 43
...
*
      ...
      ENDIF
 1    FORMAT('****WARNING**** (function ZBQLUAB) Upper and ',
     +'lower limits on uniform',/'distribution are identical')
      END
******************************************************************
      FUNCTION ZBQLEXP(MU)
*
...
* 
      IF (PPP.GT.TINY) THEN
 30    G = G + DBLE(ZBQLGEO(PPP))
       IF (G.LE.DBLE(NN)) THEN
        IZ = IZ + 1
        GOTO 30
       ENDIF
      ENDIF

      IF (PP.GT.0.5D0) IZ = NN - IZ
      ZBQLBIN = ZBQLBIN + IZ

 1    FORMAT('****ERROR**** Illegal parameter value in ZBQLBIN')
      END
******************************************************************
      FUNCTION ZBQLGEO(P)
*
...
*
 ...
 1    FORMAT('****ERROR**** Illegal parameter value in ZBQLGEO')
      END
******************************************************************
      FUNCTION ZBQLPOI(MU)
*
...
*
 ...
 1    FORMAT('****ERROR**** Illegal parameter value in ZBQLPOI')
      END
******************************************************************
...
891    ZBQLGAM=-log((g+exp(1.0d0))*(1.0d0-u)/(g*exp(1.0d0)))
       if (v.gt.ZBQLGAM**(g-1.0D0)) goto 889
...
50    U1=ZBQLU01(0.0D0)
      ...
      X=V/(U**R)
      IF (X.LE.M) GOTO 50
      TEST = ((X-M)**((G-1.0D0)/(R+1.0D0)))*EXP(-(X-M)/(R+1.0D0))
      IF (U.LE.TEST) THEN
       ZBQLGAM = (X-M)/H
      ELSE
       GOTO 50
 ...
 1    FORMAT('****ERROR**** Illegal parameter value in ',
     +' ZBQLGAM',/'(both parameters must be positive)')
...
*
      ...
      IF ( (NU1.LE.0.0D0).OR.(NU2.LE.0.0D0) ) THEN
       WRITE(MESSAGE,1)
       CALL INTPR(TRIM(MESSAGE(1)),-1,0,0)
       CALL INTPR(TRIM(MESSAGE(2)),-1,0,0)
       RETURN
...
*
 ...
 1    FORMAT('****ERROR**** Illegal parameter value in ',
     +' ZBQLBET1',/'(both degrees of freedom must be positive)')
...
*
 ...
 1    FORMAT('****ERROR**** Illegal parameter value in ',
     +' ZBQLWEI',/'(both parameters must be positive)')
      END
***************************************************************
      FUNCTION ZBQLNB(R,P)
*
...
*
 ...
 1    FORMAT('****ERROR**** Illegal parameter value in ZBQLNB')
      END
***************************************************************
